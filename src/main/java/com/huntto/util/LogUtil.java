...
public class LogUtil {
	private LogUtil() {
		...
		HANDLER_CONSOLE = false; // *** 控制台输出
		HANDLER_FILE = true; // *** 文件输出
		...
		if (HANDLER_FILE) {
			...
			// LOG_FILE_PATH = new
			// File(Log.class.getClassLoader().getResource(".").getFile()).toString() +
			// "/log/";
			// *** 2. 如果是web项目，LOG文件夹在项目根目录下D:\IMG
//            LOG_FILE_PATH = Thread.currentThread().getContextClassLoader().getResource("").getPath()+"log/";
			LOG_FILE_PATH = "D:\\IMG\\log\\";

			FILE_SIZE = 2000000; // *** 每个文件最大2M
			FILE_MAX = 10; // *** 每天最多生成10个文件,最多不可超过100

			LOG_FILE_LASTNAME = ".txt"; // *** 文件后缀名
		}
	}
			...
			handler.setFormatter(new Formatter() {
				...
				public String format(LogRecord record) {
					...
					formatMessage.append(" ").append(element.getFileName()).append("(Line:")
							.append(element.getLineNumber()).append("):").append(" ").append(record.getMessage())
							.append("\n");
